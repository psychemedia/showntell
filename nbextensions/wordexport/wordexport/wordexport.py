...
def bundle(handler, model):
	...
	curr_dir = os.path.dirname(abs_nb_path)
	
	# Generate HTML version of file
	cmd='jupyter nbconvert --to html "{abs_nb_path}" --output-dir "{tmp_dir}"'.format(abs_nb_path=abs_nb_path,tmp_dir=curr_dir)
	#os.system(cmd)
	subprocess.check_call(cmd, shell=True)
	
	staged=os.path.join(curr_dir, notebook_name)
	# Convert to MS Word .docx
	cmd='cd "{tmp_dir}" && pandoc -s "{staged}.html" -o "{staged}.docx"'.format(tmp_dir=curr_dir, staged=staged)
	#os.system(cmd)
	subprocess.check_call(cmd, shell=True)
